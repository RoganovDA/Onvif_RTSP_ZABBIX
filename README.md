...
1. Для одновременной загрузки шаблона и дашбордов воспользуйтесь файлом `zbx_export_bundle.yaml` и меню **Administration →
   General → Import**.
2. Если нужен только шаблон, импортируйте `zbx_export_templates.yaml` через **Configuration → Templates → Import**.
3. После импорта назначьте шаблон хост-группе `Security Cameras - AllCamers` или собственной группе камер.
4. Убедитесь, что на сервере присутствует внешний скрипт `camcheck.py` и он доступен пользователю `zabbix`.
5. Перед массовым применением запустите проверку синтаксиса YAML командой:
...
- `zbx_export_bundle.yaml` — единый экспорт, включающий шаблон и оба дашборда (подходит для общего импорта).
...
1. Чтобы избежать ошибки «unexpected tag "dashboards"» и загрузить всё сразу, воспользуйтесь файлом `zbx_export_bundle.yaml` вместе с меню **Administration → General → Import**.
2. При поэтапном импорте загрузите шаблон `zbx_export_templates.yaml` через **Configuration → Templates → Import**.
3. Затем импортируйте `zbx_export_dashboards.yaml` через **Monitoring → Dashboards → Import** — этот файл нельзя загружать через меню шаблонов, иначе появится описанная ошибка.
4. Убедитесь, что в окне импорта отмечено действие «Create new», чтобы новые сущности были добавлены, а не заменены.
